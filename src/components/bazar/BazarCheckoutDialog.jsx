...
export default function BazarCheckoutDialog({ product, walletGold = 0, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-t-2xl border border-border bg-card text-foreground shadow-2xl sm:rounded-2xl"
      ...
      >
        ...
        <div className="flex gap-2 border-t border-border px-4 py-3">
          <button onClick={onClose} className="flex-1 rounded-xl border border-border bg-secondary py-2.5 text-xs font-medium">
          ...
          {!result?.ok && (
            <button
              ...
              className="flex-1 rounded-xl bg-primary py-2.5 text-xs font-semibold text-primary-foreground disabled:opacity-50"
            >
              {submitting ? (
