...
export default function ConversationSidebar({ messages, onLoadConversation, onNewConversation, userEmail }) {
  ...
  const loadConversations = async () => {
    setLoading(true);
    try {
      const filters = userEmail ? { tag: 'conversation', created_by: userEmail } : { tag: 'conversation' };
  ...
  const saveConversation = async () => {
    ...
    try {
      ...
      toast({ title: 'Conversation saved', description: 'You can reopen it from Jackie at any time.' });
    } catch (error) {
      toast({ title: 'Save failed', description: error?.message || 'Could not save this conversation.', variant: 'destructive' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const renameConversation = async (id) => {
    if (!editingTitle.trim() || busy) return;
    setBusy(true);
    try {
      ...
      toast({ title: 'Rename failed', description: error?.message || 'Could not rename this conversation.', variant: 'destructive' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const deleteConversation = async (id) => {
    ...
    try {
      ...
      toast({ title: 'Delete failed', description: error?.message || 'Could not delete this conversation.', variant: 'destructive' });
    } finally {
      setBusy(false);
  ...
  const openConversation = (item) => {
    try {
      ...
      toast({ title: 'Conversation loaded', description: item.title || 'Saved conversation' });
    } catch {
      onLoadConversation([]);
      toast({ title: 'Conversation data is invalid', description: 'The saved transcript could not be restored.', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="w-full md:w-80 md:min-w-80 border-b md:border-b-0 md:border-r border-border bg-card/60 backdrop-blur">
          ...
          <div className="max-h-[12rem] overflow-y-auto overscroll-contain touch-pan-y space-y-2">
            {loading ? (
              ...
              <div key={item.id} className="rounded-xl border border-border bg-secondary/10 p-3">
                <button onClick={() => openConversation(item)} className="w-full text-left text-xs font-semibold text-foreground truncate">
        ...
        <div className="p-4 space-y-3">
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Save and revisit your Jackie chats.</p>
            </div>
            <button
          ...
          {loading ? (
            ...
            <div key={item.id} className="rounded-xl border border-border bg-secondary/10 p-3">
              <div className="flex items-start gap-2">
                <button onClick={() => openConversation(item)} className="flex min-w-0 flex-1 items-start gap-2 text-left">
      ...
      <ConfirmDialog
        open={!!pendingDelete}
        title={`Delete ${pendingDelete?.title || 'this conversation'}?`}
        description="This removes the saved transcript from Jackie. Your current open chat will stay in place until you start a new one."
        confirmLabel="Delete"
        tone="danger"
        busy={busy}
