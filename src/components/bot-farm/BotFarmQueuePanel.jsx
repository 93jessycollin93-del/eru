  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-4 space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-foreground">Task Queue</p>
          <p className="text-[11px] text-muted-foreground">Prioritize real work by urgency, fit, risk, and output value.</p>
        </div>
        <select value={sortMode} onChange={(e) => setSortMode(e.target.value)} className="rounded-xl border border-border bg-background px-3 py-2 text-xs text-foreground outline-none">
          ...
          <option value="fit">Bot fit</option>
        ...
        {sorted.map((task) => (
          <div key={task.id} className="rounded-xl border border-border bg-background p-3">
            ...
            <div className="mt-3 flex justify-end">
              <button onClick={() => onAssignTask(task)} className="rounded-xl bg-primary px-3 py-2 text-[11px] font-semibold text-primary-foreground">Assign Task</button>
            </div>
          </div>
        ))}
      </div>
